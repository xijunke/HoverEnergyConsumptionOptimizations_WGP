function wing_para_output=wing_shape_fruitfly_sixteen_1()
% 创建时间——2014年6月19日,0:16:04
% 修改时间——2014年6月14日,23:35:03
% (1)正确的无量纲弦长计算——计算有误——XXX;
% (2)正确的有效力臂的无量纲位置Z_nd计算——计算有误——XXX;
% (3)针对扭转轴为翅根和翅尖的连线时——翅根坐标与翅平面坐标系原点不重合——XX;
% clear all;clc;
...
% yr_leadbem=-0.08249*x_bem.^6+0.9167*x_bem.^5-4.04*x_bem.^4+8.872*x_bem.^3-10.06*x_bem.^2+5.674*x_bem-0.413; % yr_leadbem =1.0958;  
% yr_trailbem=-0.0333*x_bem.^6+0.504*x_bem.^5-2.795*x_bem.^4+7.258*x_bem.^3-8.769*x_bem.^2+3.739*x_bem+0.1282; % yr_trailbem =-0.0298; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%
%% 方案(1)
%% 求解——无量纲弦长分布多项式函数——计算有误——XX
r_nd1=linspace(0,1,200);    % r=r_nd*R_wingeff;
Cr_nd1=C_rx/C_aver;          % 无量纲弦长分布  
% cftool
% 针对r_nd1和Cr_nd1采用函数cftool调用拟合工具箱进行无量纲弦长分布函数拟合，获得如下K阶多项式
% 输出结果: Linear model Poly6: f(x) = p1*x^6 + p2*x^5 + p3*x^4 + p4*x^3 + p5*x^2 + p6*x + p7
% Coefficients (with 95% confidence bounds):
% p1 =-40.83  (-40.83, -40.83);p2 =  87.21  (87.21, 87.21); p3 =-59.43  (-59.43, -59.43);
% p4 =11.86  (11.86, 11.86);  p5 =-3.754  (-3.754, -3.754);  p6 = 4.938  (4.938, 4.938);
...
% Z_rnd=double(int(yr_nd1,r_nd,0,1));                         % 输出: Z_nd=0.15341;  %  mm
% 方案(2)——由前缘函数的无量纲化yr_leadnd和无量纲化弦长分布C_nd求得
% y0=yr_trailnd;                     % 此时输出Z_rnd=0.1534; ——见方案(1)
y0=yr_nd+yr_leadnd-C_nd;    % 此时输出Z_rnd=0.1402;
y1=yr_nd+yr_leadnd;
yr_nd2=(y1^4+y0^4)/4;   % 注意这里的扭转轴直接通过翅尖和翅根的连线，前缘函数的翅膀展向区间唯一
Z_rnd=double(int(yr_nd2,r_nd,0,1));     % 输出: Z_rnd2=0.1402;
...
F_yrotcoeff=(1/2)*Rou*R_wingeff^2*C_avereff^2*F_ndRot;  % 单位是mg*mm
...
I2y=Rou*C_avereff^2*R_wingeff^3*I2/2;           % 单位是 mg.mm^2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
I_xzam=pi*Rou*C_avereff^3*R_wingeff^2*I3/4;  % 单位是 mg.mm^2;   % I_xzam =0.002;
...
I_xxam=pi*Rou*C_avereff^4*R_wingeff*I4/4;      % 单位是 mg.mm^2;   % I_xxam = 0.00062892;
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (3) 虚质量气动力参数——I5z
...
I6z=pi*Rou*C_avereff^3*R_wingeff*I6/4;            % 单位是 mg.mm
