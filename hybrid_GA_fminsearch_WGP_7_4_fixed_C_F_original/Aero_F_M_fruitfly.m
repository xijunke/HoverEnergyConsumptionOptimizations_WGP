...
% %%%%%%%%%%%%%%%%%%%%%%%%%
% wing_para=wing_shape_fruitfly_sixteen_1(); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% (3) 用于计算拍打ODE方程的方案%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% wing_kenimatics=kenimatics_wing_and_AoA_fruitfly_exp_Cfcoeff();
f=188.7;  % f=234;
T=1/f;
...
F_yrot=C_R*omega_x.*V_nonr*F_yrotcoeff*10^(-3);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
F_inert_y=-m_wing*(domega_z*x_com-domega_x*z_com+omega_y.*(omega_x*x_com+omega_z*z_com))*10^6;  % 翅膀自身惯性力—法向 % 单位是uN
F_inert_z=-m_wing*(-domega_y*x_com-omega_y.^2*z_com+omega_x.*(omega_z*x_com-omega_x*z_com))*10^6; % 翅膀自身惯性力—弦向 % 单位是uN
%% (2)最新惯性力计算方案%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% m_wing=2.4*10^-3;                                 % mg
% % g=9.821;                                                % m/s^2=N/kg=uN/mg
% xr=0.3289;                                                %  \mm       % x-root offset 
% x_com=xr+1.920243385;                         %  \mm       % 质心的展向坐标
% % z_com=-0.149785466+0.636=0.486215;% 到扭转轴的弦向距离
% z_com=0.149785466;                               %  \mm       % 到扭转轴的弦向距离
% %%%%%%%%%%%%%%%%%%%%%%%%%
% F_inert_y=-m_wing*(domega_z*x_com-domega_x*z_com+omega_y.*(omega_x*x_com+omega_z*z_com))*10^-3;  % 翅膀自身惯性力—法向 % 单位是uN
% F_inert_z=-m_wing*(-domega_y*x_com-omega_y.^2*z_com+omega_x.*(omega_z*x_com-omega_x*z_com))*10^-3; % 翅膀自身惯性力—弦向 % 单位是uN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% xlswrite('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\optimal\optimal_wing_para\Forcenormal_3T.xlsx',B,'sheet1','A1:D1000');
...
%%%%%%%%%%%%%%%%%%%%%%%%%不含惯性力，水平方向和垂直方向的力
plot(t/T,F_ytran,'b-',t/T,F_yrot,'b:',t/T,F_yadd1,'g-.',t/T,F_inert_y,'c-.',t/T,F_inert_z,'m-.','LineWidth',2.5)
legend('^{rw}\itF_{y,tran}','^{rw}\itF_{y,rot}','^{rw}\itF_{y,add}','^{rw}\itF_{y,inertia}','^{rw}\itF_{z,inertia}')
%%%%%%%%%%%%%%%%%%%%%%%%%
xlabel('Normalized time','FontSize',20,'FontName','Times','FontWeight','Bold')
...
for i=1:1:N
    %调用函数COP_Ycpnd2_TransCirc求解净压心的无量纲位置Y_rcpnd; % 正负交替
    % Y_rcpnd_trans(i,1)=COP_Ycpnd2_TransCirc(alpha2(i,1),R_wing,xr0,C_maxyaxis); % 用于优化——发AST&JA论文用
     Y_rcpnd_trans(i,1)=COP_Ycpnd2_TransCirc_WGP_optimization(alpha2(i,1),R_wing,xr0,C_maxyaxis);  % 用于发BB论文用
     % Y_rcpnd_trans(i,1)=COP_Ycpnd2_fruitfly3(alpha2(i,1)); % 用于AM论文显示力矩输出
...
for i=1:1:N
    ...
    % 压心分布符合Dickinson函数:
   % Y_rcpnd_rot(i,1)=COP_Ycpnd2_RotCirc(alpha2(i,1),R_wing,xr0,C_maxyaxis);  % 用于优化——发AST&JA论文用
   Y_rcpnd_rot(i,1)=COP_Ycpnd2_RotCirc_WGP_optimization(alpha2(i,1),R_wing,xr0,C_maxyaxis); % 用于发BB论文用
   % Y_rcpnd_rot(i,1)=COP_Ycpnd2_fruitfly3(alpha2(i,1)); % 用于AM论文显示力矩输出
...
z_com=0.149785466;                               %  \mm       % 到扭转轴的弦向距离
%%%%%%%%%%%%%%%%%%%%%%%%%
M_inert_x=-z_com*m_wing*(ddphi.*cos(psi)*x_com-ddpsi*z_com+dphi.^2.*(sin(2*psi)/2)*z_com)*10^(-3);  % uN.mm
M_inert_y=x_com*m_wing*(-ddphi.*sin(psi)*x_com-dpsi.^2*z_com-dphi.^2.*(sin(psi)).^2*z_com)*10^(-3);  % uN.mm
M_inert_z=-x_com*m_wing*(ddphi.*cos(psi)*x_com-ddpsi*z_com+dphi.^2.*(sin(2*psi)/2)*z_com)*10^(-3);  % uN.mm
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(101)
plot(t/T,M_xtrans,'r--',t/T,M_xrotcirc,'b-',t/T,M_xrd,'g--',t/T,M_xam,'c-',t/T,M_inert_x,'m-','LineWidth',2.5)
legend('^{rw}\itM_{x,trans}','^{rw}\itM_{x,rot}','^{rw}\itM_{x,rd}','^{rw}\itM_{x,add}','^{rw}\itM_{x,inert}')
xlabel('Normalized time','FontSize',20,'FontName','Times','FontWeight','Bold')
...
axis([t(1,1)/T,t(1,1)/T+1,min(M_xam)-0.5,max(M_xam)+0.5])
box on
set(gca,'YGrid','off','XTick',(min(t/T):0.025:min(t/T)+1),'LineStyle','-','LineWidth',1.5,'FontSize',16,'FontName','Times','FontWeight','Bold')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 绕扭转轴的功率计算
% 平动环量力矩功率  % uW
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(103)
plot(t/T,M_ztrans,'r--',t/T,M_zrot,'b-',t/T,M_zrd,'g--',t/T,M_zadd,'c-',t/T,M_inert_z,'m-',t/T,M_inert_y,'m--','LineWidth',2.5)
legend('^{rw}\itM_{z,trans}','^{rw}\itM_{z,rot}','^{rw}\itM_{z,rd}','^{rw}\itM_{z,add}','^{rw}\itM_{z,inert}','^{rw}\itM_{y,inert}')
xlabel('Normalized time','FontSize',20,'FontName','Times','FontWeight','Bold')
ylabel('Aerodynamic moments  ( \muN.mm )','FontSize',20,'FontName','Times','FontWeight','Bold')
...
figure(105)
plot(t/T,P_Ix_inertia,'r--',t/T,P_Iz_inertia,'b-.',t/T,P_I_inertia,'k-','LineWidth',2.5)
xlabel('Normalized time','FontSize',20,'FontName','Times','FontWeight','Bold')
...
% xlabel('Normalized time','FontSize',20,'FontName','Times','FontWeight','Bold')
ylabel('Pitch power output ( \muW )','FontSize',20,'FontName','Times','FontWeight','Bold')
...
axis([min(t/T),min(t/T)+1,-inf,inf])
box on
set(gca,'YGrid','off','XTick',(0.9:0.025:2.05),'LineStyle','-','LineWidth',1.5,'FontSize',14,'FontName','Times','FontWeight','Bold') 
%%%%%%%%%%%%%%%%%%%%%%%%%
figure(107)
...
xlabel('Normalized time','FontSize',20,'FontName','Times','FontWeight','Bold')
ylabel('Flapping power output ( \muW )','FontSize',20,'FontName','Times','FontWeight','Bold')
