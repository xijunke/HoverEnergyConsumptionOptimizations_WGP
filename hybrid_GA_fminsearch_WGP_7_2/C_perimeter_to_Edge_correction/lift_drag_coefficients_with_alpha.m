...
% lambda=0.6685; % lambda=0.6676;
% lambda=0.7923; % lambda=0.7925;
% lambda=0.7491; % lambda=0.7484; 
% lambda=[0.6685,0.9133]; % 模式搜索优化出来的结果 % stdev_C_F_vari =0.0531;
lambda=[0.615,0.9325];  % 试探出这个与实验测试而得的气动力系数较接近 % stdev_C_F_vari =0.0730;
lambda_1=lambda(1,1);  % lambda=0.61;
lambda_2=lambda(1,2);  % lambda_2=0.925;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% size(C_L_dickinson)
stdev_C_L_vari=std((C_L_dickinson'-C_L_nabawy'),0,1); % stdev_C_L_vari=0.0381;
stdev_C_D_vari=std((C_D_dickinson'-C_D_nabawy'),0,1); % stdev_C_D_vari=0.1078;
stdev_C_F_vari=(stdev_C_L_vari+stdev_C_D_vari)/2; % stdev_C_F_vari=0.0730;
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot(alpha_4,C_L_dickinson,'r*',alpha_4,C_D_dickinson,'b*',...
       alpha_4,C_L_nabawy,'k-',alpha_4,C_D_nabawy,'k-','LineWidth',2.5);
...
legend('\itC_{L,Dickinson}','\itC_{D,Dickinson}','\itC_{L,Nabawy}','\itC_{D,Nabawy}')
